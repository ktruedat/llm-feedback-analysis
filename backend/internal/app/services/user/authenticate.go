...
func (s *svc) AuthenticateUser(ctx context.Context, req *requests.LoginUserRequest) (string, *user.User, error) {
	...
	token, authenticatedUser, err := s.authenticateUser(ctx, req, spanLogger)
	if err != nil {
	...
	return token, authenticatedUser, nil
}
...
func (s *svc) authenticateUser(
	...
	if u.IsDeleted() {
		return "", nil, errors.ErrUnauthorized("account has been deleted")
	}

	if !u.IsActive() {
		return "", nil, errors.ErrUnauthorized("account is not active")
	}
